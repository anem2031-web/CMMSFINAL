    ...
    {/* ===== TRIAGE DIALOG ===== */}
      ...
      <div id="print-task-section" className="hidden print:block">
        ...
        <div className="print-header">
          <h1>مهمة صيانة</h1>
          <div className="po-num">{ticket.ticketNumber}</div>
        </div>

        <div className="info-grid">
          <div className="info-box">
            <h3>تفاصيل البلاغ</h3>
            <div className="info-item"><span className="info-label">تاريخ الإنشاء:</span><span className="info-value">{new Date(ticket.createdAt).toLocaleString("ar-SA")}</span></div>
            <div className="info-item"><span className="info-label">الحالة:</span><span className="info-value">{getStatusLabel(ticket.status)}</span></div>
            <div className="info-item"><span className="info-label">الأولوية:</span><span className="info-value">{getPriorityLabel(ticket.priority)}</span></div>
            <div className="info-item"><span className="info-label">التصنيف:</span><span className="info-value">{getCategoryLabel(ticket.category)}</span></div>
          </div>
          <div className="info-box">
            ...
            <div className="info-item"><span className="info-label">مقدم البلاغ:</span><span className="info-value">{reportedBy?.name || "-"}</span></div>
            <div className="info-item"><span className="info-label">الفني المسند:</span><span className="info-value">{assignedTo?.name || "-"}</span></div>
          </div>
        </div>

        <div className="section-header">وصف المشكلة</div>
        <div className="content-block">
          <span className="content-label">{getField(ticket, "title")}</span>
          <div className="content-text">{getField(ticket, "description") || "لا يوجد وصف إضافي"}</div>
        </div>

        {(ticket.inspectionNotes || ticket.repairNotes || ticket.materialsUsed) && (
          <>
            <div className="section-header">تفاصيل التنفيذ</div>
            {ticket.inspectionNotes && (
              <div className="content-block">
                <span className="content-label">ملاحظات الفحص:</span>
                <div className="content-text">{ticket.inspectionNotes}</div>
              </div>
            )}
            {ticket.repairNotes && (
              <div className="content-block">
                <span className="content-label">ملاحظات الإصلاح:</span>
                <div className="content-text">{getField(ticket, "repairNotes")}</div>
              </div>
            )}
            {ticket.materialsUsed && (
              <div className="content-block">
                <span className="content-label">المواد والمهمات المستخدمة:</span>
                <div className="content-text">{ticket.materialsUsed}</div>
              </div>
            )}
          </>
        )}
        ...
        {linkedPOs.length > 0 && (
          <>
            <div className="section-header">طلبات الشراء المرتبطة</div>
        ...
        <div className="footer-note">
          تم استخراج هذا التقرير آلياً من نظام إدارة الصيانة (CMMS) بتاريخ {new Date().toLocaleString("ar-SA")}
        </div>
      </div>
    </>
  );
